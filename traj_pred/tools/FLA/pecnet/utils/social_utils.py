...
def collect_data(set_name, dataset_type='image', batch_size=512, time_thresh=48, dist_tresh=100, scene=None,
    ...
    rel_path = f'/data/lfh/THUDv2/datasets/eth/{set_name}'

    full_dataset = []
    full_masks = []

    current_batch = []
    mask_batch = [[0 for i in range(int(batch_size * 1.5))] for j in range(int(batch_size * 1.5))]  # [1.5*bs, 1.5*bs]
    ...
    for file in glob.glob(rel_path + part_file):
        ...
        all_data_dict = data_by_id.copy()  # {"1.0":[n1,person_id, frame_id, x, y]; "2.0":[n2,person_id, frame_id, x, y] ...}
        if verbose:
        ...
        while len(list(data_by_id.keys())) > 0:
            ...
            current_signal = bool(len(all_data_dict[curr_keys[0]]) >= 20)
            if current_signal:
                current_batch.append(all_data_dict[curr_keys[0]][:20])
                related_list.append(current_size)
                current_size += 1

            for i in range(1, len(curr_keys)):
                if social_and_temporal_filter(curr_keys[0], curr_keys[i], all_data_dict, time_thresh,
                                              dist_tresh) and len(all_data_dict[curr_keys[i]]) >= 20:
                    current_batch.append(all_data_dict[curr_keys[i]][:20])
                    related_list.append(current_size)
                    current_size += 1
...
def generate_pooled_data(b_size, t_tresh, d_tresh, train=True, scene=None, verbose=True):
    if train:
        full_train, full_masks_train = collect_data("train", batch_size=b_size, time_thresh=t_tresh, dist_tresh=d_tresh,
                                                    scene=scene, verbose=verbose)
        train = [full_train, full_masks_train]
        train_name = "../social_pool_data/train_{0}_{1}_{2}_{3}.pickle".format(
            'all' if scene is None else scene[:-2] + scene[-1], b_size, t_tresh, d_tresh)
        with open(train_name, 'wb') as f:
            pickle.dump(train, f)

    if not train:
        full_test, full_masks_test = collect_data("test", batch_size=b_size, time_thresh=t_tresh, dist_tresh=d_tresh,
                                                  scene=scene, verbose=verbose)
        test = [full_test, full_masks_test]
        test_name = "../social_pool_data/test_{0}_{1}_{2}_{3}.pickle".format(
...
class SocialDataset(data.Dataset):
    ...
    def __init__(self, set_name="train", b_size=4096, t_tresh=60, d_tresh=50, scene=None, id=False, verbose=True):
        'Initialization'
        load_name = "../social_pool_data/{0}_{1}{2}_{3}_{4}.pickle".format(set_name,
                                                                           'all_' if scene is None else scene[:-2] +
                                                                                                        scene[-1] + '_',
                                                                           b_size, t_tresh, d_tresh)
        print(load_name)
...
if __name__ == '__main__':    
    ...
    full_train, full_masks_train = collect_data('test_office', batch_size=4096, time_thresh=0, dist_tresh=100)
    train = [full_train, full_masks_train]
    train_name = f'../social_pool_data/social_office_test_4096_0_100.pickle'
    with open(train_name, 'wb') as f:
        pickle.dump(train, f)
