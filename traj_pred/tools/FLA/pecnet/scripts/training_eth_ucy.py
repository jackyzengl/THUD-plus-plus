...
def train(train_dataset):
    ...
    for i, (traj, mask, initial_pos) in enumerate(zip(train_dataset.trajectory_batches, train_dataset.mask_batches, train_dataset.initial_pos_batches)):
        ...
        x = x.to(device)
        dest = y[:, -1, :].to(device)  # [bs, 2]
        future = y[:, :-1, :].contiguous().view(y.size(0),-1).to(device)  # [bs, (12-1)*2]
...
if __name__ == '__main__':
    ...
    for subset in dada_datasets:
        ...
        train_dataset = SocialDatasetETHUCY(set_name=subset, set_type='train', b_size=hyper_params["train_b_size"], t_tresh=hyper_params["time_thresh"], d_tresh=hyper_params["dist_thresh"])
        test_dataset = SocialDatasetETHUCY(set_name=subset, set_type='test', b_size=hyper_params["test_b_size"], t_tresh=hyper_params["time_thresh"], d_tresh=hyper_params["dist_thresh"])
        ...
        train_losses = np.zeros([hyper_params['num_epochs']])
        ...
        np.save(f'../saved_models/{subset}_loss.npy', train_losses)
