...
sys.path.append('/data/lfh/DADA/tools/FLA/sgan')
from sgan.data.loader import data_loader
from sgan.losses import gan_g_loss, gan_d_loss, l2_loss
from sgan.losses import displacement_error, final_displacement_error

from sgan.models import TrajectoryGenerator, TrajectoryDiscriminator, Discriminator_inlay
...
def main(args):
    ...
    train_path = f'/data/lfh/DADA/datasets/{args.dataset_name}/train_origin'
    val_path = f'/data/lfh/DADA/datasets/{args.dataset_name}/val'
    ...
    len_source, len_target = len(train_loader), len(val_loader)
    ...
    d_inlay = Discriminator_inlay(args.decoder_h_dim_g, 2*args.decoder_h_dim_g).cuda()
    ...
    optimizer_d_inlay = optim.Adam(d_inlay.parameters(), lr=args.g_learning_rate)
    ...
    while t < args.num_iterations:
        ...
        for index_batch in range(len_max):
            ...
            if d_steps_left > 0:
                step_type = 'd'
                losses_d = discriminator_step(args, batch_s, generator,
                ...
                step_type = 'g'
                losses_g = generator_step(args, batch_s, batch_t, generator,
                                          discriminator, d_inlay, g_loss_fn,
                                          optimizer_g, optimizer_d_inlay)
...
def generator_step(
    args, batch_s, batch_t, generator, discriminator, d_inlay, g_loss_fn, optimizer_g, optimizer_d_inlay
):
    ...
    loss_mask_s = loss_mask_s[:, args.obs_len:]

    for _ in range(args.best_k):
        generator_out = generator(obs_traj_s, obs_traj_rel_s, seq_start_end_s)
    ...
    scores_fake = discriminator(traj_fake, traj_fake_rel, seq_start_end_s)
    ...
    hidden_s = generator.get_hidden_state(obs_traj_s, obs_traj_rel_s, seq_start_end_s)
    score_fake = d_inlay(hidden_s)
    hidden_t = generator.get_hidden_state(obs_traj_t, obs_traj_rel_t, seq_start_end_t)
...
if __name__ == '__main__':
    ...
    for subset in datasets:
        ...
        args.output_dir = '../checkpoint/checkpoint_FLA_10'
